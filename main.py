...
y_offsets = [2.5, 1.25, .5, -1.25, -.5, -3, -2.5, -1.75, 
             0, -1, -1, -2, 0, -.25, .25, 0,
...
target_y_list = [-5, -6, -7.5, -7, -6.5, -6.8, -5.3, -4.5,
...
additional_wrist_offsets = [0, 0, +4, +50, +40, 0, 0, 0, 
                            0, 0, 0, +30, +25, 0, 0, 0,
                            0, 0, 0, +10, +12, 0, 0, 0,
                            0, 0, 0, +20, +12, 0, 0, 0,
                            0, 0, 0, -5, +10, 0, 0, 0,
...
def infer_chess_board(starting_images):
    ...
    for starting_image in starting_images:
        if starting_image is None:
            print_timestamped("Error: No image.")
            return None
        else:
...
def send_to_arduino(data, label, list = True, not_first = False): # 1 is right arm, 0 is left arm
    ...
    if list == True:
        ...
        message = "hi" #arduino.readline().decode().strip()
        if message:
