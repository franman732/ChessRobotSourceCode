...
arduino = serial.Serial(port='COM8', baudrate=9600, timeout=1) # 1 is right arm, 0 is left arm, 2 is tradeoff arm.
...
servo_baselines_left = [365, -50, 500, 509, 134, 500]  #Index 1, servo 2, actually counts as an offset here, not a baseline, but it is easier to just put it here than to add an extra variable for it
default_servo_positions = [350, 495, 0, 274, 850, 511] #This is the default positions the servos reset to each time
...
Tradeoff_left_arm_positions = [0, 700, 225, 444, 121, 750] # This is the position for the left arm to be in to tradeoff
...
x_offsets = [1.5, .5, -.5, 0, -2, 1, .5, 1, 
             -0.5, -1.5, -1.5, -4, -4.5, -1.5, -1.7, -1.25, 
...
y_offsets = [2.5, 1.25, .5, 0, 0, -3, -1.25, -1.75, 
             0, -1, -1, -2, 0, -1, .25, 1.25,
...
target_y_list = [-5.5, -6, -7.5, 0, -7, -6.8, -5.3, -4.5,
...
additional_wrist_offsets = [0, 0, +4, 0, +40, 0, 0, 0, 
                            0, 0, 0, +30, +40, 0, 0, 0,
                            0, 0, 0, +20, +12, 0, 0, 0,
                            0, 0, 0, +20, +12, 0, 0, 0,
                            0, 0, 0, +12, 0, 0, 0, 0,
...
piece_values = {"empty": 0, "Pawn": 1, "Rook": 5, "Knight": 3, "Bishop": 3, "Queen": 9, "King": 9}
    ...
    if skip_move == 0:
        ...
        if (previous_board != board) and (board.is_checkmate() == False) and (board.is_stalemate() == False) and (board.turn != human):
            if check_1 == True:
                ...
                temp_positions = determine_positions() # Updates initial characteristics 
                previous_positions = temp_positions
                check_1 = False
            else:
                initial_characteristics = current_characteristics
...
def send_to_arduino(data, label): # 1 is right arm, 0 is left arm
    print("label:", label, "Sending to Arduino:", data)
    for value in data:
...
def move_to_exchange_position(arm, type): #Once at default position, moves arms to exchange position for pick or place, and returns to default, type is "pick" or "place"
    print("move_to_exchange_position: arm:", arm, "type:", type)
...
def tradeoff_arm_grab(from_arm, type): # Makes tradeoff arm grab or place piece, starting from default position and ending at default position
    print("tradeoff_arm_grab: from_arm:", from_arm, "type:", type)
...
def exchange_pieces(from_arm, to_arm): # Starts with arm having piece already picked up
    print("Exchanging pieces from arm", from_arm, "to arm", to_arm)
...
def score_squares(square):
    ...
    left_square = square - 1 if square % 8 != 0 else None
    right_square = square + 1 if square % 8 != 7 else None
    up_square = square - 8 if square >= 8 else None
    down_square = square + 8 if square < 56 else None

    left_piece = board.piece_at(left_square) if left_square is not None else None
    right_piece = board.piece_at(right_square) if right_square is not None else None
    up_piece = board.piece_at(up_square) if up_square is not None else None
    down_piece = board.piece_at(down_square) if down_square is not None else None

    horizontal_score = (piece_values[left_piece] if left_piece is not None else 0) + (piece_values[right_piece] if right_piece is not None else 0)
    vertical_score = (piece_values[up_piece] if up_piece is not None else 0) + (piece_values[down_piece] if down_piece is not None else 0)
...
def determine_wrist_rotation(servo_6_val, direction):
    ...
    if direction == "vertical":
        potential = 333 + servo_6_val # I think this value is wrong; fix later
        if potential > 1000:
            return servo_6_val - 333
        else:
            return potential

def move_to_highest_position(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines, servo_2_direction): # this function puts the arm at the highest positoin to then be moved downwards
    global default_servo_positions
    ...
    positions.append(determine_wrist_rotation(servo_6_val, servo_2_direction) + servo_baselines[1]) # wrist rotation
...
def move_upwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines, servo_2_direction): # this function goes from lowest position up to highest position
    global default_servo_positions
...
def move_arm_to_square(square_num, type, arm): # This function starts the entire movement section; starts with arm at default, grabs piece, returns to default
    print("move_arm_to_square: we are moving arm:", arm, "to square:", square_num)
    if arm == 1:
    ...
    move_to_highest_position(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines, servo_2_direction)
    ...
    time.sleep(.5)
    move_upwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines, servo_2_direction)
...
def move_arm_to_remove_piece(arm, side = None): # This function starts from default position with piece, moves to capture position and drops off piece, and then moves back to default position; based off arm passed, it determines capture positions but does not do transfer
    ...
    positions = []
    print("move_arm_to_remove_piece: we are removing piece with arm:", arm, "and capture positions are:", capture_positions_right if arm == 1 else capture_positions_left)
    if (arm == 0 or arm == 1):
    ...
    print("move_arm_to_remove_piece: capture positions are:", capture_positions)
    positions.append(arm)
...
def capture(from_square, from_arm): # Starts from defualt, picks up piece, moves it off board, and then ends at default position
    ...
    print("we are in capture and from_arm is:", from_arm, "and capture positions are:", capture_positions_right if from_arm == 1 else capture_positions_left)
    if from_arm == 1:
        capture_positions = capture_positions_right
    elif from_arm == 0:
        capture_positions = capture_positions_left
    print("capture positions are:", capture_positions)
...
def determine_type_of_robot_move(move):
    ...
    if move.promotion is True:
        print("Move is promotion:", move, "promotion thingi:", move.promotion)
        return "promotion"
    elif board.is_en_passant(move) is True:
        print("Move is en passant:", move, "en passant thingi:", board.is_en_passant(move))
        return "en passant"
    elif board.is_capture(move) is True:
        print("Move is capture:", move, "capture thingi:", board.is_capture(move))
        return "capture"
    elif board.is_castling(move) is True:
        print("Move is castling:", move, "castling thingi:", board.is_castling(move))
        return "castling"
    else:
        print("move is normal:", move)
        return "normal"
...
while board.is_checkmate() == False and board.is_stalemate() == False: 
    if check_for_move():
        ...
        print("Move type:", move_type, "From square:", f, "To square:", t, "From arm:", from_arm, "To arm:", to_arm, "Swap needed:", swap)
    ...
    if board.is_checkmate():
        print("Checkmate! Winner:", "Black" if board.turn == "Black" else "White")
