...
move_counter = 0 # This tracks every time the arm moves up and down; down is 1, up is another 1
...
def send_to_arduino(data, label, speed = 1000): # 1 is right arm, 0 is left arm
    #print("label:", label, "Sending to Arduino:", data)
    for value in data:
        arduino.write(bytes(f"{value},", 'utf-8'))
    arduino.write(bytes(f"{speed},", 'utf-8'))
...
def score_squares(square):
    ...
    print("left_piece:", left_piece, "right_piece:", right_piece, "up_piece:", up_piece, "down_piece:", down_piece)
...
def determine_wrist_rotation(servo_6_val, direction): # Type in this case is "empty" or "piece", and movement is "up" or "down" 
        print("direction:", direction, "servo_6_val:", servo_6_val, "potential 1", servo_6_val - 333, "potential 2", servo_6_val + 333)
...
def move_to_highest_position(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines, servo_2_direction): # this function puts the arm at the highest position to then be moved downwards
    global default_servo_positions, move_counter
