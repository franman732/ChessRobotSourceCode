...
Tradeoff_arm_tradeoff_right = [0, 758, 893, 291, 654, 757] # Left and right here are from the perspective of the human
Tradeoff_arm_tradeoff_left = [0, 758, 893, 297, 660, 248] #these are the positions for the tradeoff arm to be in to grab the pieces on the tradeoff platform

Tradeoff_right_arm_positions = [570, 235, 211, 441, 122, 235] # This is the position for the right arm to be in to tradeoff
Tradeoff_left_arm_positions = [0, 750, 225, 444, 121, 750] # This is the position for the left arm to be in to tradeoff
...
arduino_delay = .035 #This is the delay in milliseconds between sending commands to the Arduino to give it ample time to interpret previous set of commands
...
def determine_move():
    ...
    return result.move.from_square, result.move.to_square, result.move
...
def move_to_exchange_position(arm, type): #Once at default position, moves arms to exchange position for pick or place, and returns to default, type is "pick" or "place"
    ...
    global default_servo_positions, Tradeoff_arm_tradeoff_right, Tradeoff_arm_tradeoff_left, Tradeoff_left_arm_positions, Tradeoff_right_arm_positions
    ...
    positions[4] = tradeoff_positions[4] # Moves shoulder to final position and puts down piece
    send_to_arduino(positions, "exchange_positions")
    time.sleep(1)
...
def tradeoff_arm_grab(from_arm, type): # Makes tradeoff arm grab or place piece, starting from default position and ending at default position
    ...
    global tradeoff_arm_default_positions, Tradeoff_left_arm_positions, Tradeoff_right_arm_positions
    if from_arm == 1:
        tradeoff_positions = Tradeoff_right_arm_positions.copy()
    elif from_arm == 0:
        tradeoff_positions = Tradeoff_left_arm_positions.copy()
...
def move_to_highest_position(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines): # this function puts the arm at the highest positoin to then be moved downwards
    global default_servo_positions
...
def move_downwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines): # this function goes from highest position down to lowest position
    global default_servo_positions
...
def move_upwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines): # this function goes from lowest position up to highest position
    global default_servo_positions
...
def move_arm_to_square(square_num, type, arm): # This function starts the entire movement section; starts with arm at default, grabs piece, returns to default
    ...
    move_to_highest_position(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines)
    move_downwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines)
    ...
    time.sleep(1.5)
    move_upwards(target_x, target_height, servo_6_val, square_num, type, arm, servo_baselines)
...
def initialize_game():
    ...
    move_to_default(2, "pickup")
    time.sleep(.035)
    move_to_default(1, "pickup")
    time.sleep(.035)
    move_to_default(0, "pickup")
